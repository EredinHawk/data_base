...
{
    "city_depart"   : "Город_отправления",
    "city_dest"     : "Город_назначения",
    "date"          : "Дата в формате 2024-06-20"
}

Описание - результат обработки будет варьироваться в зависимости от тела запроса.
Два варианта:

1. На фронте не определены параметры поиска. 
То есть клиент, зайдя на сайт, не внес желаемые параметры 
поиска авиарейсов и конечная точка вернет все возможные варианты.
...
(Уверен это является BAD PRACTICE, но пока это самое простое до чего я додумался,
чтобы реализовать многофункциональность одной конечной точки)

2. Фронт определил параметры для поиска.
В данном случае обязательные поля тела запроса будут содержать в себе данные, 
к примеру, введенные из списков городов, или календаря для указания даты вылета.
