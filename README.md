...
С использованием языка программирования Go реализуем API, который будет поддерживать всего лишь одну конечную точку Fligths и четыре возможных метода:

#### GET

HTTP запрос, отправленный на ручку Flights с методом GET, реализует поиск авиабилетов в БД и вернет их список в формате JSON в теле HTTP ответа. 
