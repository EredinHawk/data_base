...
Общая схема API представлена на диаграмма выше.

#### Конечные точки API

1. Flights
```Bash
URL         - localhost:8090/flights
HTTP метод  - GET
Тело        - обязательно в формате:

{
    "city_dest"   : "Город_отправления",
    "city_depart" : "Город_назначения",
    "date"        : "Дата в формате 2024-06-20"
}

Описание - результат обработки будет варьироваться в зависимости от тела запроса. Два варианта:

1. На фронте не определены параметры поиска. 
То есть клиент, зайдя на сайт, не внес желаемые параметры поиска авиарейсов и конечная тока вернет все возможные варианты рейсов.
В данном случае тело запроса должно выглядеть следующим образом:
{
    "city_dest"   : "empty",
    "city_depart" : "",
    "date"        : ""
...
(Уверен это является BAD PRACTICE, но пока это самое простое до чего я додумался, чтобы реализовать многофункциональность одной конечной точки)

2. Фронт определил параметры для поиска.
В данном случае обязательные поля тела запроса будут содержать в себе данные, к примеру, введенные из списков, или календаря для указания даты вылета.
Тело запроса в таком случае будет выглядеть так:
{
    "city_dest"   : "Москва",
    "city_depart" : "Казань",
    "date"        : "2024-06-10"
